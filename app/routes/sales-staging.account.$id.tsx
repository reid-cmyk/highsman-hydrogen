...
function OrgLogo({website, name, size = 36}: {website: string | null; name: string; size?: number}) {
  ...
  if (domain && !failed) {
    return (
      <img src={`https://www.google.com/s2/favicons?domain=${domain}&sz=64`} alt={name} onError={()=>setFailed(true)}
        style={{width:size,height:size,borderRadius:'7px',objectFit:'contain',background:'#1a1a1a',flexShrink:0,imageRendering:'auto'}} />
    );
  }
  return (
